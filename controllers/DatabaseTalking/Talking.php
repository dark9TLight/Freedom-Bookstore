...
class Talking
{
    ...
    private static $pass = '';
    private static $dbname = 'bookstoredb';
    ...
    public function getAvailableBooks ()
    {
        try {
            $this->stat = $this->conn->prepare("
             SELECT a.name as author, b.book_id,b.title,b.sell_price,b.ISBN,b.buy_price,p.name as category_name,p.category_id,b.quantity,b.actual_quantity,b.description,b.image
             FROM author a JOIN book b ON a.author_id=b.author_id 
                 JOIN category p on b.category_id = p.category_id
             WHERE quantity>0 ");
            $this->stat->execute();
            while ($row = $this->stat->fetch(PDO::FETCH_ASSOC)) {
                $category = new Categorys($row['category_name'], $row['category_id']);
                new Book(
                    $row['book_id'],
                    $row['title'],
                    $row['ISBN'],
...
WHERE a.name like ? AND  b.quantity>0;");
    ...
    public function getBookInfo ($id): Book
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.book_id,b.title,b.ISBN,b.sell_price,b.buy_price,b.image,b.quantity,b.actual_quantity,b.description,a.name as author,p.name as category_name
FROM book b,author a,category p
WHERE b.book_id=?
...
");
            ...
            return new Book(
                ...
                $row['quantity'],
                new Author($row['author'], ''),
                new Categorys($row['category_name'])
            );
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            die();
    ...
    public function getNotAvailableBooks ()
    {
        try {
            $this->stat = $this->conn->prepare("
SELECT b.book_id,b.title,b.sell_price,b.buy_price,b.description,b.actual_quantity,p.phone as category,p.phone as category_phone,sum(od.quantity) as co
from book b,order_details od,orders o,category p
where o.order_date <= TIMESTAMPADD(YEAR ,1,o.order_date)
...
ORDER BY sum(od.quantity) DESC ;");
    ...
    public function getBooksOffCategory (int $pubid): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT b.book_id,b.title,b.image,b.sell_price,b.buy_price,b.ISBN
            FROM category p JOIN book b on p.category_id = b.category_id
            WHERE p.category_id=? AND b.quantity>0;");
...
/// they do not test
    ...
    public function getAllcategorys (): PDOStatement
    {
        try {
            $this->stat = $this->conn->prepare("
            SELECT category_id,name FROM category ORDER BY  2 ");
            $this->stat->execute();
            return $this->stat;
        } catch (PDOException $exception) {
            echo $exception->getMessage();
    ...
    public function getCategory ($id): Categorys
    {
        try {
            $this->stat = $this->conn->prepare("
            ...
            $this->stat->execute();
            $row = $this->stat->fetch(PDO::FETCH_ASSOC);
            return new Categorys($row['name'], $row['category_id']);
        } catch (PDOException $exception) {
            echo $exception->getMessage();
            return NULL;
        }
    ...
    public function getCategorybyname ($name): Categorys
    {
        try {
            $this->stat = $this->conn->prepare("
