<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "bookstoredb3");
		if(!$conn){
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT book_id, book_image FROM books ORDER BY book_id DESC";
	...
	function getBookByIsbn($conn, $isbn){
		$query = "SELECT book_title, book_author, book_price FROM books WHERE book_id = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getbookprice($isbn){
		$conn = db_connect();
		$query = "SELECT book_price FROM books WHERE book_id = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getPubName($conn, $pubid){
		$query = "SELECT category_name FROM category WHERE categoryid = '$pubid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
